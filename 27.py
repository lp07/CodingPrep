...
class Solution:
    def removeElement(self, nums, val) -> int:
        ...
        for n in range(len(nums)):
            # current element is equals to the given value
            if nums[n] == val:
                continue  # if it is equals, skip the remaining code inside the loop
            # if it is not equals, assign to the position i
            nums[i] = nums[n]
